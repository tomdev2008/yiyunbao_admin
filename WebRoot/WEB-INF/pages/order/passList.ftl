...
<body>
        ...
        url="/admin/order/query?status=1" allowResize="true" idField="id" multiSelect="true" allowAlternating="true" pageSize="20" > 
        <div property="columns">
        	<div name="action" width="150" align="center" headerAlign="center"  renderer="onActionRenderer">操作</div>
...
<script>
    ...
    function onActionRenderer(e) {
        ...
        var s = ' <a   href="javascript:cancelOrder(\'' + uid + '\')">取消订单</a>'
       		    + ' <a   href="javascript:showVie(\'' + uid + '\')">抢单情况</a>' ;
        return s;
    }
    ...
    /**
     * 删除订单,暂未使用
     * @param  {[type]} row_uid [description]
    ...
    function cancelOrder(row_uid) {
        var row = grid.getRowByUID(row_uid);
        if (row) {
             mini.confirm("确定取消订单？", "确定？",
                function (action) {            
                    if (action == "ok") {
                         grid.loading("处理中，请稍候......");
                        $.ajax({
                            url: "/admin/order/cancelOrder/" + row.id,
                            success: function (text) {
                                grid.reload();
                            },
                            error: function () {
                                mini.alert("取消订单失败，请稍后再试");
                                grid.unmask();
                            }
                        });
                    } 
